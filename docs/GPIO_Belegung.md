...
| :--- | :--- | :--- | :--- |
| **3** | `TOF_XSHUT` | Output | Reset/Shutdown Pin für VL53L0X (ToF) Distanzsensor. |
...
| **7** | `FAN2_PWM` | PWM Output | PWM-Steuersignal für Lüfter 2 (Zusatzlüfter). |
...
| **14** | `I2C2_SCL` | I2C Clock | SCL für I2C Bus 1 (`Wire1`). |
...
| **20** | `I2C2_SDA` | I2C Data | SDA für I2C Bus 1 (`Wire1`). **Achtung:** Konfliktpotenzial mit USB D+! |
| **21** | `FAN_PWM` | PWM Output | PWM-Steuersignal für Lüfter 1 (Hauptlüfter). |
...
*   **Wire1 (I2C1):** GPIO 20 (SDA), GPIO 14 (SCL)
    *   Verwendet für: SHT41 (Innen), GP8211 (DAC/LED), DS3231 (RTC)
    *   *Warnung:* GPIO 20 liegt auf dem internen USB PHY (D+). Nutzung kann USB-Upload/Serial stören.
