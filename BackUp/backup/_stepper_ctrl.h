...
/*
  StepperCtrl – ESP32-S3 + TMC2209 (Adafruit 0.11 Ω) + AccelStepper
  -----------------------------------------------------------------
  - Nicht-blockierende State-Machine (tick()-basiert)
  - Sensorloses Homing via SG_RESULT (Baseline + EMA + Mindestweg + Backoff)
  - Softlimits (0 .. max_travel_mm), logisch ↔ motorisch mappbar
  - Komfort-API ohne logicalDir (home/rehome, goTop/goBottom, moveUp/moveDown, jogUp/jogDown/jogStop)
  - Debug-Layer mit sinnvollen Logs (Homing, Position, Ziel, Softlimits, Jog)
*/
...
struct TMC2209Config {
  ...
  uint8_t  sg_thrs_diag = 240;   // DIAG only (Fehler), NICHT Stall
};

struct HomingParams {
  float    speed_fast_hz     = 2500.0f;
  float    speed_slow_hz     = 2000.0f;  // backoff
  float    backoff_mm        = 3.0f;
  uint32_t sg_ignore_ms      = 50;
  uint32_t sg_poll_ms        = 10;
  uint32_t sg_baseline_ms    = 200;
  uint16_t sg_abs_thr        = 0;        // 0=aus
  float    sg_drop_pct       = 0.35f;
...
struct StepperStatus {
  ...
  bool  isMoving    = false;
  bool  isHoming    = false;
  bool  lastOpDone  = false;  // letzte Ziel-/Jog-/Homing-Op fertig?
};
...
public:
  ...
  void  setDebugMoveLogInterval(uint16_t ms); // default 100 ms
...
private:
  ...
  enum class HomeState { IDLE, FAST, BACKOFF, DONE };
  ...
  void startContinuousLogical_(float speedHz, int logicalDir);
...
private:
  ...
  // Debug: Positions-Logs throtteln
